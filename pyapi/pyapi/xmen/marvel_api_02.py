...
def marvelcharcall(stampystamp, hashyhash, pkeyz, lookmeup, howmany):
    r = requests.get(XAVIER+"?name="+lookmeup+"&ts="+stampystamp+"&apikey="+pkeyz+"&hash="+hashyhash+"&limit="+howmany)

    print(XAVIER+"?name="+lookmeup+"&ts="+stampystamp+"&apikey="+pkeyz+"&hash="+hashyhash+"&limit="+howmany)
    return r.json()
...
def main():           
    ...
    uncannyxmen = marvelcharcall(nightcrawler, cerebro, storm, args.hero, args.limit)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--limit', \
      help='Number to limit the result set to the specified number of resources.')
